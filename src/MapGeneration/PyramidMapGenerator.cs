...
public static class PyramidMapGenerator
{
	public static MapData Create()
	{
		...
		foreach (var level in Enumerable.Range(0, pyramidLevels))
		{
			...
			mapData.Vertices.AddRange(
				new[] { new Vertex(-pos, -pos), new Vertex(pos, -pos), new Vertex(pos, pos), new Vertex(-pos, pos) }
			);
		...
		mapData.Vertices.AddRange(
			new[] { new Vertex(-32, -8), new Vertex(+32, -8), new Vertex(+32, +8), new Vertex(-32, +8) }
		);
		...
		mapData.LineDefs.AddRange(
			new[]
			{
				new LineDef(
					v1: switchVertexStart,
					v2: switchVertexStart + 1,
					sideFront: switchFrontSD,
					sideBack: switchInsideSD,
					twoSided: true,
					special: 11
				),
				new LineDef(
					v1: switchVertexStart + 1,
					v2: switchVertexStart + 2,
					sideFront: switchSideSD,
					sideBack: switchInsideSD,
					twoSided: true
				),
				new LineDef(
					v1: switchVertexStart + 2,
					v2: switchVertexStart + 3,
					sideFront: switchSideSD,
					sideBack: switchInsideSD,
					twoSided: true
				),
				new LineDef(
					v1: switchVertexStart + 3,
					v2: switchVertexStart,
					sideFront: switchSideSD,
					sideBack: switchInsideSD,
					twoSided: true
				),
		...
		mapData.Things.Add(new Thing(type: 1, x: 0, y: 0));
