...
public static class UdmfModelGenerator
{
	public static void WriteToPath(string basePath)
	{
		...
		{
			using (
				var blockStream = File.CreateText(
					Path.Combine(basePath, block.CodeName.ToPascalCase() + ".Generated.cs")
				)
			)
...
namespace SectorDirector.Core.FormatModels.Udmf"
				...
				output.OpenParen();
				output.Line($"[GeneratedCode(\"{CurrentLibraryInfo.Name}\", \"{CurrentLibraryInfo.Version}\")]");
				output.Line($"public sealed partial class {block.CodeName}");
				output.OpenParen();
	...
	private static void WriteCloneMethod(IndentedWriter output, Block block)
	{
		output
			...
			.OpenParen()
			.Line($"return new {block.CodeName}(")
			.IncreaseIndent();
		...
		output.DecreaseIndent().CloseParen();
	}
	...
	private static void WriteConstructors(IndentedWriter sb, Block block)
	{
		...
		foreach (var indexed in block.OrderedProperties().Select((param, index) => new { param, index }))
		{
			...
			var argument =
				$"{property.ConstructorArgumentType} {property.ConstructorArgumentName}"
		...
		foreach (var property in block.OrderedProperties())
		{
			switch (property)
			{
				...
				case BlockList blockList:
					sb.Line(
						$"{blockList.PropertyName}.AddRange({blockList.ConstructorArgumentName} ?? Enumerable.Empty<{blockList.SingularName}>());"
					);
					break;
			}
		}
	...
	{
		sb.Line(@"public Stream WriteTo(Stream stream)").OpenParen().Line("CheckSemanticValidity();");
		...
		foreach (var field in block.Fields.Where(_ => _.IsRequired))
		{
			sb.Line($"stream.WriteProperty(\"{field.FormatName}\", {field.FieldName}, indent: {indent});");
		}
		...
		sb.Line("return stream;").CloseParen();
	}

	private static void WriteSemanticValidityMethods(IndentedWriter output, Block block)
	{
		output.Line(@"public void CheckSemanticValidity()").OpenParen();
