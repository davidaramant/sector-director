...
namespace SectorDirector.Core.FormatModels.Udmf.Parsing"
	...
	{
		output.Line("static partial void ProcessGlobalAssignment(MapData map, Assignment assignment)").OpenParen();
	...
	private static void WriteBlockParsing(IndentedWriter output)
	{
		...
		foreach (
			var block in UdmfDefinitions
				.Blocks.Single(b => b.CodeName.ToPascalCase() == "MapData")
				.SubBlocks.Where(b => b.IsRequired)
		)
		{
			output
	...
	private static void WriteBlockParser(Block block, IndentedWriter output)
	{
		...
		output.CloseParen().Line($"return {variable};").CloseParen().Line();
	}

	private static void WriteFieldSwitch(IndentedWriter output, Block block, string variable)
	{
		output.Line("switch (assignment.Name.ToLower())").OpenParen();
		...
		foreach (var field in block.Fields)
		{
			output
				...
				.IncreaseIndent()
				.Line(
					$"{variable}.{field.PropertyName} = Read{field.PropertyType.ToPascalCase()}Value(assignment, \"{block.CodeName}.{field.PropertyName}\");"
				)
				.Line("break;")
				.DecreaseIndent();
		}
		...
		output
			...
			.IncreaseIndent()
			.Line(
				$"{variable}.UnknownProperties.Add(new UnknownProperty(assignment.Name, assignment.ValueAsString()));"
			)
			.Line("break;")
			.DecreaseIndent()
			.CloseParen();
