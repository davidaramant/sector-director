...
public class CollidingThing : IColliding
{
	...
	public void Move(ref Vector2 direction, float desiredDistance)
	{
		...
		bool collides = FindNearestEdgeCollision(
			CurrentSectorId,
			desiredDistance,
			ref direction,
			ref nearestEdgeCollisionPoint
		);
		...
		foreach (Line line in currentSector.Lines)
		{
			...
			if (portalId >= 0)
			{
				...
				if (heightDiff < ClimbableHeight && VerticalPosition + Height < mapInfo.HeightCeiling)
				{
					...
					float unusedX = 0,
						unusedY = 0;
					bool collidesWithSectorBoundary = CircleCollidesWithLine(
						out unusedX,
						out unusedY,
						line,
						Position,
			...
			if (canCollideWithLine)
			{
				float firstX = 0;
				float secondX = 0;
				bool collidesWithLine = CircleCollidesWithLine(
					out firstX,
					out secondX,
					line,
					potentialPosition,
	...
	private bool CircleCollidesWithLine(
		out float firstX,
		out float secondX,
		Line line,
	...
	)
	{
		...
		float c = (float)(
			...
			+ (-2 * (circleCenter.X * line.Vertex1.X + circleCenter.Y * line.Vertex1.Y))
			- Math.Pow(radius, 2)
		);
		...
		// We intersect a line at two points. Finding the midpoint of these two points is where the tip of the closest circle can get
	...
	{
		return _possibleSectorsToEnter.FirstOr(
			sectorId => _map.IsInsideSector(sectorId, ref Position),
			CurrentSectorId
		);
	}
}
