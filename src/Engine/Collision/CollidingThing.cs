...
namespace SectorDirector.Engine.Collision
{
    public class CollidingThing : IColliding
    {
            ...
            foreach (Line line in currentSector.Lines)
            {
                ...
                if (portalId >= 0)
                {
                    ...
                    if (heightDiff < ClimbableHeight && VerticalPosition + Height < mapInfo.HeightCeiling)
                    {
                        ...
                        bool collidesWithSectorBoundary = CircleCollidesWithLine(out unusedX, out unusedY, line, Position, Radius);
                ...
                if (canCollideWithLine)
                {
                    float firstX = 0;
                    float secondX = 0;
                    bool collidesWithLine = CircleCollidesWithLine(out firstX, out secondX, line, potentialPosition, Radius);
                    if (collidesWithLine)
                    {
        ...
        private bool CircleCollidesWithLine(out float firstX, out float secondX, Line line, Vector2 circleCenter, float radius)
        {
