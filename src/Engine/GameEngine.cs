	...
	private IRenderer CreateRenderer(RendererType type)
	{
		switch (type)
		{
			case RendererType.LineTest:
				return new LineTestRenderer(_settings, _screenMessage);
			case RendererType.Overhead:
				return new OverheadRenderer(_settings, _currentMap);
			case RendererType.FirstPerson:
				return new FirstPersonRenderer(_settings, _currentMap, _screenMessage);
			case RendererType.Fire:
				return new FireRenderer();
			case RendererType.MapHistory:
				return new MapHistoryRenderer(_currentMap, _screenMessage);
			default:
