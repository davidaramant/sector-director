...
public sealed class OverheadRenderer : IRenderer
{
	...
	public void Render(IScreenBuffer screen, PlayerInfo player)
	{
		...
		{
			screen.PlotCircle(
				_camera.WorldToScreen(center).ToPoint(),
				(int)(radiusInWorldCoordinates * _camera.TotalZoom),
				c
			);
		}
		...
		foreach (var lineDef in _map.Map.LineDefs)
		{
			...
			var lineColor = lineSurroundsPlayerSector
		...
		DrawLineFromWorldCoordinates(
			player.Position,
			player.Position
				+ GeometryExtensions.Rotate(
					player.Direction,
					-0.5f * player.CameraSettings.FieldOfView * MathShortcuts.DegToRad
				) * player.CameraSettings.MaxClippingDistance,
			Color.AntiqueWhite
		);
		DrawLineFromWorldCoordinates(
			player.Position,
