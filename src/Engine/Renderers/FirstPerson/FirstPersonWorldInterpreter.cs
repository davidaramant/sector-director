...
public sealed class FirstPersonWorldInterpreter
{
	...
	) ConvertWorldPointToScreenDegrees(Vector3 point)
	{
		...
		float centerAngleDiff =
			MathShortcuts.AngleBetween(new Vector2(0, 1), targetedVectorDirection) * MathShortcuts.RadToDeg;
	...
	)
	{
		...
		if (
			point1Result.isFacing
			&& point1Result.isInView
			&& !point2Result.isInView
			&& !point2Result.isFacing
			&& vector2.X < 0
		)
		{
		...
		var showBoth =
			isCloseEnough
			&& isFarEnough
			&& (point1Result.isInView || point2Result.isInView || (point1Result.isFacing && point2Result.isFacing));
