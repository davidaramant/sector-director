	...
	public void Render(IScreenBuffer screen, PlayerInfo player)
	{
		...
		foreach (SectorInfo sector in _map.Sectors)
		{
			foreach (Line line in sector.Lines)
			{
				...
				Vector3 topLeftConverted = _camera.WorldToPerspective(
					new Vector3(line.Vertex1.X, line.Vertex1.Y, sector.Info.HeightCeiling)
				);
				Vector3 topRightConverted = _camera.WorldToPerspective(
					new Vector3(line.Vertex2.X, line.Vertex2.Y, sector.Info.HeightCeiling)
				);
				Vector3 bottomLeftConverted = _camera.WorldToPerspective(
					new Vector3(line.Vertex1.X, line.Vertex1.Y, sector.Info.HeightFloor)
				);
				Vector3 bottomRightConverted = _camera.WorldToPerspective(
					new Vector3(line.Vertex2.X, line.Vertex2.Y, sector.Info.HeightFloor)
				);

				var topLineResult = Interpreter.ConvertWorldLineToScreenPoints(
					screen,
					topLeftConverted,
					topRightConverted
				);
				...
				var bottomLineResult = Interpreter.ConvertWorldLineToScreenPoints(
					screen,
					bottomLeftConverted,
					bottomRightConverted
				);
				...
				var leftLineResult = Interpreter.ConvertWorldLineToScreenPoints(
					screen,
					topLeftConverted,
					bottomLeftConverted
				);
				...
				var rightLineResult = Interpreter.ConvertWorldLineToScreenPoints(
					screen,
					topRightConverted,
					bottomRightConverted
				);
