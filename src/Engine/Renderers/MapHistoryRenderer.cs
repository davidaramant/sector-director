...
public sealed class MapHistoryRenderer : IRenderer
{
	...
	public void Update(ContinuousInputs inputs, GameTime gameTime)
	{
		...
		_linesToDraw = Math.Min(
			_map.Lines.Length,
			_linesToDraw + _msToDrawSpeed * gameTime.ElapsedGameTime.Milliseconds
		);
	}
	...
	public void Render(IScreenBuffer screen, PlayerInfo player)
	{
		...
		var gameToScreenFactor = Math.Min(
			desiredMapScreenBounds.X / _map.Area.X,
			desiredMapScreenBounds.Y / _map.Area.Y
		);
