...
public sealed class Line
{
	...
	public static Vector2 Intersection(ref Vector2 v1, ref Vector2 v2, ref Vector2 p1, ref Vector2 p2)
	{
	...
	public static bool IsPointOnLineSegment(ref Vector2 v1, ref Vector2 v2, ref Vector2 p) =>
		p.X <= Max(v1.X, v2.X) && p.X >= Min(v1.X, v2.X) && p.Y <= Max(v1.Y, v2.Y) && p.Y >= Min(v1.Y, v2.Y);
}
			...
			if (!lineDef.TwoSided)
			{
				lines.Add(
					new Line(
						...
						frontSide.Sector,
						lineDef.V1,
						Map.Vertices[lineDef.V1],
						lineDef.V2,
						Map.Vertices[lineDef.V2]
					)
				);
			}
			else
			{
				...
				lines.Add(
					new Line(
						...
						lineDef.V2,
						Map.Vertices[lineDef.V2],
						portalToSectorId: backSide.Sector
					)
				);
				lines.Add(
					new Line(
						...
						lineDef.V2,
						Map.Vertices[lineDef.V2],
						portalToSectorId: frontSide.Sector
					)
				);
			}
		}
