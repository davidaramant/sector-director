...
{
	public static List<Vertex> Vertex(Stream stream) => ReadEntries(stream, LumpEntryParser.Vertex, entryByteSize: 4);
	...
	public static List<Sector> Sector(Stream stream) => ReadEntries(stream, LumpEntryParser.Sector, entryByteSize: 26);

	public static List<Thing> Thing(Stream stream) => ReadEntries(stream, LumpEntryParser.Thing, entryByteSize: 10);

	private static List<T> ReadEntries<T>(Stream stream, Func<BinaryReader, T> entryParser, int entryByteSize)
	{
